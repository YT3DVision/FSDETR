...
class DETRLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if loss_gain is None:
            loss_gain = {'class': 2, 'bbox': 5, 'giou': 2, 'no_object': 0.05, 'mask': 1, 'dice': 1}
        self.nc = nc
        if use_mal | use_sl | use_svfl | use_emasl | use_emasvfl:
            use_fl = False
        self.matcher = HungarianMatcher(cost_gain={'class': 2, 'bbox': 5, 'giou': 2}, use_fl=use_fl)
        ...
        # for nwd loss
        self.nwd_loss = False  # False
        self.iou_ratio = 0.90
        
        # for wise-iou loss
        self.use_wiseiou = False  # False
        if self.use_wiseiou:
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        if self.use_wiseiou:
            ...
            # loss[name_giou] = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, GIoU=False, EIoU=True)
            # loss[name_giou] = 1.0 - bbox_inner_iou(pred_bboxes, gt_bboxes, xywh=True, GIoU=True, ratio=0.7) # Inner IoU
            loss[name_giou] = 1.0 - bbox_focaler_iou(pred_bboxes, gt_bboxes, xywh=True, EIoU=True, d=0.0, u=0.90) # Focaler IoU
